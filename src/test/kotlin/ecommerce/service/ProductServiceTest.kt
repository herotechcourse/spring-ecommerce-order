...
class ProductServiceTest {
    ...
    fun `getAll should return all products mapped to ProductResponse`() {
        val product = Product("Bar", 1, "https://test.jpg", listOf(Option("Opt", 1)), 1L)
    ...
    fun `getById should return mapped ProductResponse when product exists`() {
        val product = Product("Bar", 1, "https://test.jpg", listOf(Option("m", 1)), 1L)
    ...
    fun `create should save product and options`() {
        ...
        val product = Product("New", 2, "https://test.jpg", savedOption, 1L)
    ...
    fun `create should throw when name exists`() {
        ...
        val request = ProductRequest("Dup", 1, "https://test.jpg", listOf())
    ...
    fun `update should replace product and return response`() {
        val id = 10L
        val oldProduct = Product("Old", 1, "https://test.jpg", listOf(Option("Opt", 1)), id)
        val request = ProductRequest("Updated", 2, "https://test.jpg", listOf(OptionRequest("Opt", 1)))
        val updated = Product("Updated", 2, "https://test.jpg", listOf(Option("Opt", 1)), id)
    ...
    fun `update should throw if product not found`() {
        ...
        val request = ProductRequest("Updated", 1, "https://test.jpg", listOf())
